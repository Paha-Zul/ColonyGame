...
public class FindRandomNearbyLocation extends LeafTask{
    ...
    public void start() {
        ...
        Transform transform = this.blackBoard.myManager.getEntityOwner().getTransform();
            ...
            while(nodes.size() < 1) {
                ...
                for (int col = startX; col <= endX; col++) {
                    for (int row = startY; row <= endY; row++) {
                        ...
                        if (node != null && (col == startX || col == endX || row == startY || row == endY) && node.getTerrainTile() != null && !node.getTerrainTile().tileRef.avoid && node.getEntityList().isEmpty())
                            nodes.add(node);
                    }
                }
                radius++;
