...
public class Transform extends Component implements IDelayedDestroyable {
	 ...
	 * @param position The saveContainer position of the ownerID Entity.
	 * @param rotation The saveContainer rotation of the ownerID Entity.
	 */
	...
	public void update(float delta){
		//System.out.println(this.ownerID.name+" local in update: "+this.localPosition);
	...
	public void removeChild(Transform child){
		for(int i=this.children.size-1;i>=0;i--)
			if(child == this.children.get(i)){
				Transform tempChild = this.children.removeIndex(i); //Removes it from the child list.
...
//				tempChild.transform.setRotation(tempChild.transform.getRotation()); //Reset rotation.
	...
	public void setScale(float scale){
		...
		ComponentManager.getComponents(ownerID).scaleComponents(this.worldScale);
